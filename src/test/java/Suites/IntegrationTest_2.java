...
public class IntegrationTest_2 {
	...
	public void setUpDev(Method method) throws Exception {
		...
		driver.manage().window().maximize();
		new Login(driver, baseUrl, UserName, UserPass);
		Info.info("----------------------------------------------------------------------------------");
		String testName = method.getName(); 
		System.out.println(testName + " Started");
		//Info.info(testName + " Started");
	...
	{ 
		...
		{  
			...
			Info.info(String.format("%-50s %-20s",testName, "FAILED"));

		}
		if (testResult.getStatus() == ITestResult.SUCCESS) 
		{ 
			System.out.println(testName + " PASSED");
			Info.info(String.format("%-50s %-20s",testName, "PASSED"));
		...
		{ 
			...
			Info.info(String.format("%-50 %-20s",testName, "SKIPED"));
